...
public class AsyncTransformerTest {
    ...
    public void setUp() throws Exception {
        ...
        // set up second transformer (SYNC) which consumes text/plain and produces text/turtle
        port = findFreePort();
        transformerURI2 = "http://localhost:" + port + "/";
        server = new TransformerServer(port);
        server.start(new SimpleTransformer());

         // set up third transformer (SYNC) which consumes text/turtle and produces text/turtle
        port = findFreePort();
        transformerURI3 = "http://localhost:" + port + "/";
        server = new TransformerServer(port);
    ...
    public void turtleOnGet() throws UnsupportedEncodingException {
        Response response = RestAssured.given().header("Accept", "text/turtle")
                ...
                .get(baseURI + "?t1=" + transformerURI1 + "&t2=" + transformerURI2 + "&t3=" + transformerURI3);
    }
    ...
    public void turtlePost() {
        Response response = RestAssured.given().header("Accept", "text/turtle")
                ...
                .content("hello")
                .expect().statusCode(HttpStatus.SC_OK).header("Content-Type", "text/turtle").when()
                .post(baseURI + "?t1=" + transformerURI1 + "&t2=" + transformerURI2 + "&t3=" + transformerURI3);
